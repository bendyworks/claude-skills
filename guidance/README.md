...
| [`pull-requests.md`](pull-requests.md) | Small incremental PRs (300-line target, 400-line easy-review threshold), draft mode first, why-first cost-naming descriptions that link outside resources, a PR owns the bugs it introduces, per-thread review replies, and stacked-chain merge mechanics (deletion-driven retargeting, approval dismissal on base change). | everything |
