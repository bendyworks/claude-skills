...
| `finished-issue-housekeeping` | Post-ship cleanup once a story has shipped: finalize the plan file, prune branches, tidy memory and task lists, and run an approval-gated permission-prompt sweep that can propose additions to the project's `.claude/settings.json`. |
