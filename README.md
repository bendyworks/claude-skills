...
| `finished-issue-housekeeping` | Post-ship cleanup once a story is merged and deployed (on a deploy-on-merge project, merged): finalize the plan file, prune branches, tidy memory and task lists, and run an approval-gated permission-prompt sweep that can propose additions to the project's `.claude/settings.json`. |
