...
4. Run the verify command. The dial `verify_command` in
   `dependabot-autonomy.yml` overrides the default; if unset, use the
   project's standard "lint + test" entry point (e.g. `bundle exec rake` for
   Ruby/Rails projects with a default rake task, `npm test && npm run lint`
   for Node projects, or whatever the project's CLAUDE.md says). Output must
   be clean.
