...
The clean-and-green gate in Step 1 already runs the suite, and nothing edits the tree between Step 1 and this step -- so if that gate ran **with coverage on** against this tree, reuse its artifacts rather than re-running (in Targeted Spec Verification Mode the targeted-specs run does exactly that; if it ESCALATED, the full gate ran and full-mode behavior applies). If Step 1 ran without coverage, or was satisfied by the user's confirmation or a prior run, run the suite (or the relevant suites) with coverage on now and capture the artifacts. **Either way, the resultset must come from a run that started with `coverage/.resultset.json` (and its `.lock`) deleted** -- raw results are keyed by a per-process command name, so an entry an earlier session left under a name this run does not reproduce is never overwritten and merges into the union as stale coverage, hiding a genuinely uncovered added line. That applies to the reuse path too, and the merge below passes `ignore_timeout: true`, which switches off the age check that would otherwise have dropped such an entry: if you cannot confirm Step 1's run started clean, delete and re-run rather than trust its artifacts. Delete those two files specifically rather than all of `coverage/` -- removing `.last_run.json` silently disables a project's `maximum_coverage_drop` gate, which passes unconditionally when no last run exists. Subset coverage is not ground truth: one-hop selection can miss a spec that covers an added line transitively, so treat a subset-uncovered added line as a candidate to verify (or defer to CI's full-run patch coverage) rather than an automatic finding. Then intersect added lines with uncovered lines:

1. **Added lines** -- `git diff main...HEAD --unified=0` (or parse `+` hunks) gives the new-file line numbers per file.
2. **Uncovered lines** -- from the coverage run's machine-readable output. **Always read the tool's *merged* artifact, never per-process raw data.** A suite that forks (parallel runners, per-file loops, sharded CI) writes one raw entry per process, and in each entry the files that process never loaded are *simulated* as all zeros. Read one of those entries directly and an entire untouched file looks uncovered. Merging fixes that for any file at least one process loaded -- but a tracked file *no* process loaded is simulated as zeros in every entry, so the union still reports it wholly uncovered. That is the subset caveat above wearing a different hat, not a finding.
   - Ruby / SimpleCov -> merge before reading. `coverage/coverage.json` is already merged, but it exists only when the project enables the JSON formatter (SimpleCov adds it only when `CC_TEST_REPORTER_ID` is set), so usually only `coverage/.resultset.json` is on disk. Merge it read-only -- this neither re-runs the suite nor modifies the file:
     ...
     ```ruby
     # ruby -rsimplecov -e '...' -- run from the repo root
     SimpleCov.root(Dir.pwd)
     res = SimpleCov::ResultMerger.merge_results("coverage/.resultset.json", ignore_timeout: true)
     abort "no resultset -- re-run the suite with coverage on" if res.nil?
     abort "resultset holds no files under #{SimpleCov.root} -- wrong directory?" if res.files.empty?
     changed = `git diff main...HEAD --name-only`.split("\n")
     res.files.each do |f|
       rel = f.project_filename.delete_prefix("/")
       next unless changed.include?(rel)
       puts "#{rel}: #{f.missed_lines.map(&:line_number).inspect}"
     end
     ```

     Every line of that guard earns its place. `merge_results` returns `nil` when the path does not resolve, so without the first `abort` you get a `NoMethodError` instead of an answer. SimpleCov's root defaults to the working directory and filters out every file stored outside it, so running from the wrong directory -- or reading a resultset a container wrote with `/app/...` paths -- yields an empty file list and a silent "nothing uncovered"; pinning the root and asserting a non-empty list turns that into a loud failure. `missed_lines` rather than `coverage == 0` is what excludes `:nocov:` lines, which carry a hit count of `0` but are deliberately skipped. Filtering to the diff keeps a large project's whole file list out of the reply.

     `ignore_timeout: true` matters: SimpleCov's own merge silently drops entries older than `merge_timeout` (default 600s), so a slow per-file loop can otherwise lose its earliest processes. **`coverage.json` is subject to that same timeout** -- it is generated through the merge path *without* the override -- so on a slow suite it can under-report where the snippet above does not. Reading it directly: hit count `0` is uncovered, `null` is non-executable, and the string `"ignored"` marks a `:nocov:` skip. **Do not sum hit counts across entries by hand.** SimpleCov merges nil-wins (`nil + 0 = nil`), and a simulated entry marks far more lines executable than a real measurement does, so summing reports non-executable lines as uncovered -- on a two-process run of this plugin's own suite, summing claimed 189 uncovered lines against a true 53.
   - JS / Istanbul -> read the merged artifact, not per-worker output. Jest and Vitest already merge their workers into `coverage/coverage-final.json` and `lcov.info`; a raw nyc setup writes per-process `.nyc_output/*.json` and needs `nyc report` first. Then read `coverage/**/cobertura-coverage.xml` or `lcov.info` (`<line number=.. hits="0"/>` / `DA:line,0`). Note JS coverage is usually a **separate** run from the Ruby suite (e.g. an `npm test` invocation with coverage on) -- run it too when the diff touches JS, or the JS patch stays invisible.
...
After Phase 3 fixes, re-run coverage as part of the final gate -- fixes add lines too, and those should be covered before the branch leaves draft. Delete `coverage/.resultset.json` before that run as well: the fixes have shifted line numbers, so results carried over from the pre-fix run describe a file that no longer exists.
