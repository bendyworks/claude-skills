...
1. **Disproved -> `[disproved]`.** The code or a test shows the finding is wrong. State the evidence (`called at app/jobs/x.rb:19`). "Looks pre-existing" and "seems minor" are not disproof.
2. **Audits disagree -> test, then sort again.** When one audit's finding contradicts another's "considered but ruled out", or two findings contradict each other, never pick the more confident report. Settle it with evidence: read the line both depend on, or run a cheap experiment -- usually a mutation, breaking the thing on purpose to see whether a spec fails. Restore the tree afterwards and confirm `git status` shows nothing left over, or the suite gate's evidence goes stale. Then take the finding back to step 1.
3. **A judgment the developer owns -> `[ask]`.** Only these: **a design or architecture tradeoff**; **a behavior change a user or stakeholder would notice**, such as a changed URL or a different email; **a new validation or constraint on an existing column**, which rows already in the database may fail; **a performance change** that needs measuring first; **a single fix that would by itself take the PR past the 400-line threshold**; **two audits that still disagree** after step 2.
...
**Size.** Measure the PR once when sorting, the way the tail's step 2 does, and state the figure in the report. Judge only the single fix in question against the threshold; do not sum estimates across findings.
