...
The gauntlet runs the project's lint+test gate at two points: Phase 0 Step 1, to establish the branch is green before auditing it, and the Phase 3 tail, to establish the fixes did not break it. **Both runs are decided here, by this one list.** Step 1's precondition 3, the standing pre-approval, Step 4's patch coverage, and the Phase 3 tail point here rather than restating any of it.

Work the list top to bottom and act on the first branch that matches. Announce that branch in a sentence of its own before doing anything else, in exactly the form given, so the transcript says which branch decided the run.

1. **Red evidence in hand -- stop.** A gate result obtained on this tree that failed, whoever obtained it. Announce `Suite gate: stop (red evidence: <what>)` and stop the gauntlet: it is a *quality* pass, not a *rescue* pass, and a red branch gets fixed first, separately. This branch is ordered ahead of branch 2 so that a red result cannot be laundered into a green one -- branch 2 accepts a confirmation the user volunteers, and "gauntlet it anyway" is a confirmation. Accepting a red suite is a call only the developer gets to make, so honor an explicit one; announce it as `Suite gate: stop (red evidence: <what>) -- proceeding at the user's direction` and carry the failure into the record file's header, rather than recording the branch as satisfied.
2. **Green evidence in hand -- satisfied, run nothing.** Either a gate result this session observed on this tree, full or targeted, or a statement the user volunteered that specs and lint pass. Announce `Suite gate: satisfied (green evidence: <what>)`, naming which of the two it was.

   A result holds until a tracked file's content changes. Committing content that has already been tested is not a change, and neither is writing the bookkeeping files this skill and the plan-issue skill maintain -- the record file under `.claude/gauntlets/`, a plan file under `.claude/plans/` -- which no suite exercises. Any other tracked file changing invalidates the result, and the next decision starts again at the top of this list.

   A targeted verdict counts as evidence only on the terms the targeted-specs skill sets for it; read that skill's verdict contract rather than reinterpreting the line here. What it does not establish as green is not evidence, and falls through to branch 3.
3. **No evidence, and the gate can run -- announce it, run it, then re-enter this list with the result.** Announce `Suite gate: running (no evidence; <full|targeted>)` and run it in the same message, without waiting for permission: the standing pre-approval covers this run, and waiting for an acknowledgement that may never come is the stall this list exists to remove. Then take the result back to the top, where a pass lands on branch 2 and a failure on branch 1.

   Which gate: the targeted-specs skill (bundled in this plugin) when the project declares Targeted Spec Verification Mode, since that skill confirms the mode itself and ends with a verdict line; the project's full lint+test gate otherwise.

   Re-entering never produces a second targeted run. A verdict that leaves the precondition unestablished goes to the **full** gate -- a second targeted run selects from the same diff and returns the same result, which is a loop.
4. **The gate cannot run -- announce it and end.** Only two things put a run here, and both are evidence you can state: the user has said not to run it, or no runner could be found. "No runner" means all five of these came back empty -- a suite-runner skill at any level, the project's CLAUDE.md or rules files, a Rakefile default task, `package.json` scripts, and the test job in the project's continuous-integration config. Anything short of that is branch 3, not this branch; an unverified hunch that the gate is unrunnable is not an entry form.

   Announce `Suite gate: cannot run (no runner found)` or `Suite gate: cannot run (user declined the run)` -- the parenthetical is the entry condition verbatim, never a placeholder standing in for it -- say in the same breath what would unblock it, and end.

   **Do not make this turn on whether a user is watching.** A session has no reliable test for its own attendance, so a rule that asks when someone is present and ends otherwise fires its asking branch precisely when a session wrongly believes it can tell -- the one case where it is wrong. Ending and naming the blocker serves both worlds instead: a developer who is watching reads it as the question it is and acts at once, and a run nobody is watching leaves the reason in the transcript rather than waiting for an answer that will never come. Nothing is lost by ending at Phase 0 Step 1, where this branch fires before any work has happened. This is the last place an unattended run could still stall, so it does not.

**Running the gate.** Invoke a suite-runner skill if one is available, project-level or user-level, instead of hand-typing the command. Otherwise run the project's own gate command once, teeing all stdout and stderr to a uniquely-named log under /tmp, and answer every follow-up by grepping that log -- never re-run a suite just to re-read its output. A suite-runner skill that already captures this way satisfies the capture rule as-is; do not add a second capture on top of it.

Run it **with coverage on**, having first deleted `coverage/.resultset.json` and its `.lock`, so Step 4 can reuse the artifacts instead of paying for a second full run. Delete those two files specifically -- Step 4 explains why, and why a resultset from a run you did not start this way cannot be trusted.

**What "stop" and "end" mean depends on which run this is.** At Phase 0 Step 1 they are literal: nothing has happened yet, so the gauntlet ends and the developer starts it again once the branch is green or the gate is runnable. At the Phase 3 tail they are not, because the tail still owes the run a Phase 4 decision, and a gauntlet that ends without one is a bug by the rule in "When Phase 4 runs". There, branch 1 and branch 4 end **the gate**, not the gauntlet: announce as written, record the failure or the impossibility in the record file's header, skip the patch-coverage re-check that has nothing to read, and carry on to the tail's remaining steps so Phase 4 is still decided. Say plainly in the closing message that the post-fix gate did not pass or did not run, so "ready for human review" cannot be read as "verified green".

**What to record.** The Phase 0 decision is made at Step 1, one step before the record file exists, so carry its facts to Step 2 and write them into the header there -- Step 2 says what the header holds. The Phase 3 tail's decision is appended to that header when it is made. Record facts rather than a branch label, because the question Step 4 and the tail actually ask is not which branch decided a run; it is whether a trustworthy coverage-bearing run has happened against this tree.
...
1. Decide the gate again by "When the suite gate runs" (Phase 0), which owns this run as much as Phase 0's -- including what still counts as evidence here, since a Phase 3 that changed no tracked file leaves the Phase 0 result standing and lands on its branch 2. Then re-run the Step 4 patch-coverage check: the fixes added lines too, and those should be covered before the branch leaves draft.
