...
- **A declared GitHub issue key is not a Linear ID.** When the project's CLAUDE.md declares an issue key ("GitHub issues here are called PRJ-NNN: PRJ-NNN is issue #NNN"; the plan-issue skill, bundled in this plugin, defines the declaration), `PRJ-NNN` is GitHub issue #NNN: follow the GitHub bullet below with the bare number, not `linear`.
