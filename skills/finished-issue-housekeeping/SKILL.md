...
2. **The merged code is live in production.** In Deploy-on-Merge Mode, check 1 satisfies this one: the merge is the deploy, so there is no later event to look for. Otherwise check the project's deploy log:
   ...
   A project whose merge *triggers* a deploy that can still fail is not in Deploy-on-Merge Mode even if its rules claim otherwise. If the declaration and the deploy setup disagree, verify the deploy before proceeding and tell the user the declaration looks wrong.
