...
- **Invoke it via the Skill tool** (skill name `fewer-permission-prompts`,
  ...
  and move on.
- **Report what actually changed, from `git status` -- not from
  memory.** After the sweep, check `git status` and name the settings
  file(s) it actually edited (as of this writing the built-in targets
  the project's `.claude/settings.json`). This step can end the pass
  with an uncommitted diff, so say so plainly in the Step 10 summary.
  Committing the change stays with the user, through the project's
  normal flow -- possibly a PR, since in many projects that file is
  checked in and team-visible. Never commit or push it as part of
  this pass. Note that the diff sits on the branch checked out in
  Step 3 (usually `main`) and will otherwise ride into the next
  story's first commit, so flag it to the user rather than leaving it
  to be discovered.
